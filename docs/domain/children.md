...
- **Résumé** (écran 7) : « L’univers de {prénom} », une ligne par section (niveau, passions, objectifs, à renforcer, façons d’apprendre, confort) avec un crayon vers l’écran concerné ; une fois le résumé atteint, enregistrer un écran y ramène. L’observation privée n’y figure pas. « Découvrir mon espace » termine l’onboarding (puis l’espace parent exige l’adresse vérifiée) ; « Ajouter un autre enfant » le termine et en ouvre aussitôt un nouveau. Le bouton « Créer son premier exercice » de la maquette attendra la génération d’activités (étape 5).
...
- `tests/Feature/Children/OnboardingTest.php`, `OnboardingInterestsGoalsTest.php`, `OnboardingDifficultiesPreferencesTest.php` et `OnboardingSummaryTest.php` : écrans 2 à 7, validation, retour sans perte, progression, reprise, accès.
